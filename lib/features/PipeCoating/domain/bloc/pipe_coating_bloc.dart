...
import 'package:bsppl/features/PipeCoating/domain/bloc/pipe_coating_event.dart';
import 'package:bsppl/features/PipeCoating/domain/bloc/pipe_coating_state.dart';
import 'package:bsppl/features/PipeCoating/helper/pipe_coating_helper.dart';
...
class PipeCoatingBloc extends Bloc<PipeCoatingEvent, PipeCoatingState>{
  ...
  File get photo => _photo;
   ...
   TextEditingController pipeNoController = TextEditingController();
  ...
  _submit(PipeCoatingSubmitEvent event, emit) async {
    try{
      var validationCheck = await PipeCoatingHelper.validation(
          ...
          pipeNo: reportNumberController.text.trim().toString(),
      );
      if(await validationCheck == true){
        ...
        var res = await PipeCoatingHelper.submitData(
          ...
          pipeNumber: pipeNoController.text.trim().toString(),
  ...
  _eventComplete(Emitter<PipeCoatingState>emit) {
    emit(PipeCoatingFetchDataState(
        ...
        pipeNoController: pipeNoController,

...
import 'package:bsppl/features/Welding/Engineer/helper/welding_helper.dart';
...
class WeldingBloc extends Bloc<WeldingEvent, WeldingState>{
  ...
  fetchWpsType({required BuildContext context}) async{
    var res = await WeldingHelper.wpsTypeAPIData(sectionId: sectionId,context: context);
    if(res != null){
      _wpsList = res;
    }
  }
  fetchWelder({required BuildContext context}) async{
    var res = await WeldingHelper.welderAPIData(sectionId: sectionId,context: context);
    if(res != null){
